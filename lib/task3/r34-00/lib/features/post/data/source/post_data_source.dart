...
import 'package:r34_00/features/post/data/models/post_model.dart';
import 'package:r34_00/features/post/domain/entity/post.dart';

abstract class AbstractPostDataSource {
  void addPost({required Post post});
  void deletePost({required int id});
  List<Post> getAllPosts();
  Post getPost({required int id, required int userId});
  Post updatePost({required Post post});
}

class PostDataSource extends AbstractPostDataSource {
  // Private static instance
  static final PostDataSource _instance = PostDataSource._internal();
...
''';
  ...
  void deletePost({required int id}) {
    try {
      ...
      return posts.removeWhere((element) => element.id == id);
    } catch (e) {
      throw ServerException();
    }
  }
  ...
  List<Post> getAllPosts() {
    try {
      if (posts.isEmpty) {
        ...
        posts.addAll(
          Iterable.generate(data.length, (index) {
            return PostModel.fromJson(data[index]);
          }),
        );
      }
